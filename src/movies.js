...
//1- create a function that goes through elements inside an array (moviesArray) of objects and extract a specific element (directors)
//2-create a new array(directorsArray) to stock the elements extracted (directors) and return it.


//   ***************  code starts in line 2015  *******************
...
//1. use filter to class all the movies by steven spielberg in one array (stevenspielbergMovies)
...
// const stevenSpielbergMovies = howManyMovies(movies);
// console.log(stevenSpielbergMovies);

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
//1.access the scores inside the array objects with the same aproach done before
//2.calculate the sum of all the scores and devide it per array length
function scoresAverage(moviesArray) {
    ...
    const average = validScores / moviesArray.length;
...
//1. go through the list of movies and extract genre : drama
//2. get the movies with the genre drama into a dramaMoviesArray
//3. calculate the sum of the drama movies and devide it on the dramaArray.length
function dramaMoviesScore(moviesArray) {
    const dramaMoviesArray = moviesArray.filter(movie => typeof movie.score === 'number');
    const totalScore = dramaMoviesArray.reduce((acc, movie) => acc + movie.score, 0);
...
const result2 = dramaMoviesScore(dramaMovies);
console.log(result2);

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
//1. use the function sort
function orderByYear(moviesArray) {
    let sortedmovies =[...moviesArray].sort((a, b) => a.year - b.year);
    return moviesArray;
}
...
function orderAlphabetically(moviesArray) {
    let alphabeticSort = [...moviesArray].sort((a, b) => a.title - b.title );
    return moviesArray;

     
    const first20Titles = [...moviesArray].slice(0, 20).map(movie => movie.title);
    
    return first20Titles;
}
    const alphabeticSort =  orderAlphabetically(movies);   
